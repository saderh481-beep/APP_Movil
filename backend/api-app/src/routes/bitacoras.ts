  ...
  async (c) => {
    ...
    const [actualizada] = await sql`
      UPDATE bitacoras SET
        ...
        updated_at                = NOW()
      WHERE id = ${id}
      RETURNING id, tipo, estado, observaciones_coordinador, actividades_desc, recomendaciones, comentarios_beneficiario, fecha_inicio, fecha_fin, coord_inicio, coord_fin
    `;
